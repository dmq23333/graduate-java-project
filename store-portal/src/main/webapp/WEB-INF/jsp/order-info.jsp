...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	$(function(){
		...
		var receriverName = $("div.consignee-item").find("span.receiverName").text();
		var receiverPhone = $("div.consignee-item").find("span.receiverPhone").text();
		var receiverState = $("div.consignee-item").find("span.receiverState").text();
		var receiverCity = $("div.consignee-item").find("span.receiverCity").text();
		var receiverDistrict = $("div.consignee-item").find("span.receiverDistrict").text();
		var receiverAddress = $("div.consignee-item").find("span.receiverAddress").text();
		var receiverZip = $("div.consignee-item").find("span.receiverZip").text();
		$(input_out[0]).attr("value",receriverName);
		$(input_out[1]).attr("value",receiverPhone);
		$(input_out[2]).attr("value",receiverState);
		$(input_out[3]).attr("value",receiverCity);
		$(input_out[4]).attr("value",receiverDistrict);
		$(input_out[5]).attr("value",receiverAddress);
	});
...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		...
		<input type="hidden" class="input_style" name="orderShipping.receiverName" value="收件人姓名"/>
		<input type="hidden" class="input_style" name="orderShipping.receiverMobile" value="15891588888"/>
		<input type="hidden" class="input_style" name="orderShipping.receiverState" value="北京"/>
		<input type="hidden" class="input_style" name="orderShipping.receiverCity" value="北京"/>
		<input type="hidden" class="input_style" name="orderShipping.receiverDistrict" value="昌平区"/>
		<input type="hidden" class="input_style" name="orderShipping.receiverAddress" value="西三旗 xxxxxxxxx"/>
</form>
   			...
   			<ul class="ui-switchable-panel-main" id="consignee-list">
				...
				<li class="ui-switchable-panel" id="consignee_index_137617472"
					...
					<div class="consignee-item item-selected"
						...
						<div class="user-name">
							<div class="fl">
								<strong limit="4"><span class="receiverName">${shippingConfirm.receiverName}</span></strong>&nbsp;&nbsp;收
							</div>
							<div class="fr"><span class="receiverPhone">${shippingConfirm.receiverPhone}</span></div>
							<div class="clr"></div>
						</div>
						<div class="mt10" limit="15"><span class="receiverState">${shippingConfirm.receiverState}</span><span class="receiverCity">${shippingConfirm.receiverCity}</span></div>
						<div class="adr-m" limit="30"><span class="receiverDistrict">${shippingConfirm.receiverDistrict}</span> <span class="receiverAddress">${shippingConfirm.receiverAddress}</span></div>
						<div class="adr-m" limit="30"><span class="receiverZip">${shippingConfirm.receiverZip}</span></div>
			...
			<ul id="payment-list">
				...
				<li style="cursor: pointer;" onclick="save_Pay(4);">
					...
					<div class="payment-item  online-payment "
						...
						<b></b> 支付金额 <font class="whiteBarSpanClass hide"
...
<div class="shopping-list ABTest">
	<div class="goods-list">
     <!--配送方式-->
    <h4 class="vendor_name_h" id="0">商家：电子商城</h4>		         
    <div class="goods-suit goods-last">
	 <c:forEach items="${orderItemList }" var="orderItem">
		<div class="goods-item goods-item-extra">

			<div class="p-img">
				<a target="_blank" href="/item/${orderItem.itemId}.html">
					<img src="${orderItem.picPath}" alt=""/>
				</a>
			</div>
			<div class="goods-msg">
				<div class="p-name">
					<a href="/item/${orderItem.id}.html" target="_blank">
						${orderItem.title } 
					</a>
				</div>
				<div class="p-price">
					...
					<strong>￥<fmt:formatNumber
							...
							minFractionDigits="2" value="${orderItem.price / 100 }" /></strong>
					<!--增加预售金额显示 end-->
					<span class="ml20"> x${orderItem.num} </span> 
